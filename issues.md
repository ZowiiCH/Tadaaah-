Limites : 

Dans la première scène de bus, lorsque vous ratez votre arrêt, je n'ai pas réussi à faire apparaître la bulle rouge comme je le souhaitais. Elle disparaissait directement ou n'aparaissait pas du tout. J'ai donc choisi l'option d'implémenter un "wait", ce qui crée un petit bug lorsque l'on clique à plusieurs reprises sur la bulle avant qu'elle ne disparaisse. Le dialogue va pop autant de fois que le joueur a cliqué, cela n'engendre cependant pas de problème majeur dans le déroulement de l'expérience. 

Pistes d'amélioration : 

Dans ce petit jeu, j'ai fait le choix de présenter les quelques différents symptômes indépendant les uns des autres. L'objectif étant de permettre aux enfants de pouvoir bien les différencier individuellement. Il serait cependant intéressant de faire un jeu plus long montrant la complexité du trouble avec la superposition des symptômes, tout en ajoutant des exemples concrets des aspects positifs et des outils/mécanismes permettant de compenser le tadah sans la prise d'un traitement. 